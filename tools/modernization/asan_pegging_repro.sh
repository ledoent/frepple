...
# No `set -e`: runtest.py is expected to fail (it reproduces a crash) and we want
# to print its exit code below. Instead the build steps are guarded explicitly so
# a build failure aborts before the test runs against a stale/missing binary.
set -uxo pipefail
...
cmake -B /work/build -S /work -DCMAKE_BUILD_TYPE=Debug || exit 1
...
cmake --build /work/build -j"$(nproc)" || exit 1
